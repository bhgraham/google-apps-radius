...
/**
 * The list of domains allowed to authenticate
 * This becomes handy when you want to authenticate using google but for only certain domains
 */
var alloweddomains;
...
module.exports.authenticate = authenticate = function(username, password, domain, options, callback) {
  ...
  if( alloweddomains && alloweddomains.filter(function(d){ return d.indexOf(domain) >= 0; } ).length > 0 ){
    callback({ error: 'Domain '+domain+' not allowed. The complete list of allowed domains is ['+alloweddomains.join(',')+']'});
    return;
  }
...
module.exports.createServer = function (options) {
  ...
  alloweddomains = options.alloweddomains ? options.alloweddomains.split(',') : null;
